...
class DpsCalculator(private val dataStorage: DataStorage) {
    ...
    companion object {
        ...
        val SKILL_CODES: IntArray = SKILL_MAP.keys.sorted().toIntArray()
        val SMALL_SKILL_CODES: Set<Int> = SKILL_MAP.keys.filter { it < 1_000_000 }.toSet()
    }

    private val targetInfoMap = hashMapOf<Int, TargetInfo>()
    private val lastSkillByAttackUid = mutableMapOf<AttackKey, Int>()

    private data class AttackKey(
        val actorId: Int,
        val targetId: Int,
        val attackUid: Int
    )
    ...
    fun getDps(): DpsData {
        ...
        pdps.forEach { pdp ->
            ...
            if (dpsData.map[uid]!!.job == "") {
                val job = JobClass.convertFromSkill(resolvedSkillCode)
                if (job != null) {
    ...
    private fun resolveSkillCode(pdp: ParsedDamagePacket): Int {
        val rawSkillCode = pdp.getSkillCode1()
        val attackUid = pdp.getAttackUid()
        val key = if (attackUid != 0) {
            AttackKey(pdp.getActorId(), pdp.getTargetId(), attackUid)
        } else {
            null
        }

        if (isPlausibleSkillId(rawSkillCode)) {
            if (key != null) {
                lastSkillByAttackUid[key] = rawSkillCode
            }
            return rawSkillCode
        }

        val cachedSkill = key?.let { lastSkillByAttackUid[it] }
        if (cachedSkill != null) {
            return cachedSkill
        }
        ...
        if (inferred != null && key != null && isPlausibleSkillId(inferred)) {
            lastSkillByAttackUid[key] = inferred
        }
        return inferred ?: 0
    }
